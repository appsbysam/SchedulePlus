(() => {
  ...
  const state={deferred:null,manifestUrl:null,ready:false,image:null,imageUrl:null,drag:false,lastX:0,lastY:0,baseScale:1,scale:1,offsetX:0,offsetY:0,targetFile:null,bgColor:'#0b0b0d',pickMode:false};
  ...
  const supportedBrowser=()=>window.SchedulePlusBrowserSupported!==false;
  const browserName=()=>window.SchedulePlusBrowserName||'this browser';
  ...
  function applyTenantManifest(){
    ...
    const manifest={name,short_name:name.length>16?name.slice(0,16):name,description:`${name} job scheduling powered by Schedule+.`,id:`${base}?tenant=${encodeURIComponent(b.slug||slugify(name))}`,start_url:base,scope:base,display:'standalone',display_override:['standalone','minimal-ui'],background_color:b.app_icon_background_color||'#0b0b0d',theme_color:b.accent_color||'#0b0b0d',prefer_related_applications:false,icons:[{src:abs(iconChoice(b,192)),sizes:'192x192',type:b.app_icon_192_url?'image/png':'image/svg+xml',purpose:'any'},{src:abs(iconChoice(b,512)),sizes:'512x512',type:b.app_icon_512_url?'image/png':'image/svg+xml',purpose:'any'},{src:abs(iconChoice(b,'maskable')),sizes:'512x512',type:b.app_icon_maskable_url?'image/png':'image/svg+xml',purpose:'maskable'}]};
  ...
  function ensureUi(){
    ...
    const appearance=document.querySelector('.appearance-settings');if(!appearance)return;
    state.ready=true;
    const panel=document.createElement('section');panel.className='pwa-branding-panel';panel.innerHTML=`<div class="pwa-branding-head"><strong>App icon & installation</strong><small>Create the icon customers see on their home screen, app drawer and installed Schedule+ app.</small></div><div class="pwa-icon-row"><div class="pwa-icon-preview"><img id="pwaIconPreview" src="${DEFAULT_ICON}" alt="App icon preview"></div><div class="pwa-icon-actions"><button id="pwaCropFromLogo" class="pwa-icon-primary" type="button">Create from business logo</button><button id="pwaChooseDifferent" class="pwa-icon-secondary" type="button">Upload different image</button><input id="pwaIconFile" type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" hidden></div></div><div class="pwa-branding-help">A square icon is generated in 192×192, 512×512 and maskable formats for proper PWA installation.</div><p id="pwaBrandingStatus" class="pwa-branding-status"></p>`;
    appearance.insertAdjacentElement('afterend',panel);
    document.body.insertAdjacentHTML('beforeend',`<dialog id="pwaCropDialog" class="pwa-crop-dialog"><div class="pwa-crop-card"><div class="dialog-head"><h2>Crop app icon</h2><button id="pwaCropClose" class="ghost" type="button">✕</button></div><div id="pwaCropStage" class="pwa-crop-stage"><canvas id="pwaCropCanvas" width="1024" height="1024"></canvas><div class="pwa-safe-area" aria-hidden="true"></div></div><label class="pwa-crop-zoom">Zoom<input id="pwaCropZoom" type="range" min="0.35" max="3" step="0.01" value="1"></label><div class="pwa-bg-controls"><div><strong>Background colour</strong><small>Used whenever you zoom out and around the maskable icon.</small></div><div class="pwa-bg-actions"><button id="pwaBgAuto" class="pwa-bg-btn" type="button">Auto</button><button id="pwaBgPick" class="pwa-bg-btn" type="button">Pick from logo</button><label class="pwa-bg-custom"><span>Custom</span><input id="pwaBgColor" type="color" value="#0b0b0d"></label></div></div><p id="pwaPickHint" class="pwa-pick-hint hidden">Tap a colour in the logo preview.</p><p class="pwa-branding-help">Drag to reposition. The dotted guide shows the safe area for maskable icons.</p><div class="pwa-crop-actions"><button id="pwaCropCancel" class="secondary" type="button">Cancel</button><button id="pwaCropApply" class="primary" type="button">Save app icon</button></div></div></dialog>`);
    ...
    $('pwaCropZoom').oninput=e=>{state.scale=state.baseScale*Number(e.target.value);recenterIfNeeded(false);draw()};
    $('pwaBgColor').oninput=e=>setBackground(e.target.value);
    $('pwaBgAuto').onclick=()=>{state.pickMode=false;$('pwaPickHint').classList.add('hidden');setBackground(suggestBackground())};
    $('pwaBgPick').onclick=()=>{state.pickMode=true;$('pwaPickHint').classList.remove('hidden')};
    const stage=$('pwaCropStage');stage.onpointerdown=startDrag;stage.onpointermove=drag;stage.onpointerup=endDrag;stage.onpointercancel=endDrag;stage.onclick=pickColourFromCanvas;
  ...
  async function useBusinessLogo(){if(!canManage())return;const b=business();const src=b?.logo_dark_url||b?.logo_url||b?.logo_light_url;if(!src){alert('Upload a business logo first, or choose a different image.');return}await startCropUrl(src)}
  ...
  function loadImage(url,cross){const img=new Image();if(cross)img.crossOrigin='anonymous';state.image=img;img.onload=()=>{const c=$('pwaCropCanvas');state.baseScale=Math.max(c.width/img.naturalWidth,c.height/img.naturalHeight);state.scale=state.baseScale;state.offsetX=(c.width-img.naturalWidth*state.scale)/2;state.offsetY=(c.height-img.naturalHeight*state.scale)/2;state.bgColor=business()?.app_icon_background_color||suggestBackground()||'#0b0b0d';$('pwaBgColor').value=state.bgColor;$('pwaCropZoom').value='1';state.pickMode=false;$('pwaPickHint').classList.add('hidden');draw();$('pwaCropDialog').showModal()};img.onerror=()=>alert('That image could not be opened.');img.src=url}
  function setBackground(color){state.bgColor=/^#[0-9a-f]{6}$/i.test(color||'')?color:'#0b0b0d';if($('pwaBgColor'))$('pwaBgColor').value=state.bgColor;draw()}
  function suggestBackground(){if(!state.image)return business()?.accent_color||'#0b0b0d';try{const s=document.createElement('canvas');s.width=s.height=48;const x=s.getContext('2d',{willReadFrequently:true});x.clearRect(0,0,48,48);x.drawImage(state.image,0,0,48,48);const d=x.getImageData(0,0,48,48).data,bins=new Map();for(let i=0;i<d.length;i+=4){if(d[i+3]<180)continue;const r=Math.round(d[i]/32)*32,g=Math.round(d[i+1]/32)*32,b=Math.round(d[i+2]/32)*32,key=`${Math.min(255,r)},${Math.min(255,g)},${Math.min(255,b)}`;bins.set(key,(bins.get(key)||0)+1)}let best=null,count=0;for(const[k,v]of bins){if(v>count){best=k;count=v}}if(best){const[r,g,b]=best.split(',').map(Number);return '#'+[r,g,b].map(v=>v.toString(16).padStart(2,'0')).join('')}}catch(_){}return business()?.accent_color||'#0b0b0d'}
  function draw(){const c=$('pwaCropCanvas'),ctx=c?.getContext('2d');if(!ctx||!state.image)return;ctx.fillStyle=state.bgColor;ctx.fillRect(0,0,c.width,c.height);ctx.drawImage(state.image,state.offsetX,state.offsetY,state.image.naturalWidth*state.scale,state.image.naturalHeight*state.scale)}
  function recenterIfNeeded(force=true){if(!state.image)return;const c=$('pwaCropCanvas'),w=state.image.naturalWidth*state.scale,h=state.image.naturalHeight*state.scale;if(force||w<c.width){state.offsetX=(c.width-w)/2}if(force||h<c.height){state.offsetY=(c.height-h)/2}}
  function startDrag(e){if(!state.image||state.pickMode)return;state.drag=true;state.lastX=e.clientX;state.lastY=e.clientY;e.currentTarget.setPointerCapture?.(e.pointerId)}
  ...
  function pickColourFromCanvas(e){if(!state.pickMode)return;const c=$('pwaCropCanvas'),r=c.getBoundingClientRect(),x=Math.max(0,Math.min(c.width-1,Math.floor((e.clientX-r.left)*c.width/r.width))),y=Math.max(0,Math.min(c.height-1,Math.floor((e.clientY-r.top)*c.height/r.height)));try{const p=c.getContext('2d').getImageData(x,y,1,1).data;const hex='#'+[p[0],p[1],p[2]].map(v=>v.toString(16).padStart(2,'0')).join('');setBackground(hex)}catch(_){alert('Could not sample that colour.')}state.pickMode=false;$('pwaPickHint').classList.add('hidden')}
  function closeCrop(){state.pickMode=false;$('pwaCropDialog')?.close()}
  ...
  async function installAction(){
    ...
    const steps=$('pwaInstallSteps');if(isIos()){steps.innerHTML='<li>Tap the Share button in Safari.</li><li>Choose <strong>Add to Home Screen</strong>.</li><li>Tap <strong>Add</strong>.</li>'}else{steps.innerHTML=`<li>Open the ${browserName()} browser menu.</li><li>Choose <strong>Install app</strong> or <strong>Add to Home screen</strong>.</li><li>Confirm the installation.${supportedBrowser()?'':' The installed shortcut may show the browser badge or browser icon.'}</li>`}$('pwaInstallHelp').showModal()
  }
  window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();state.deferred=e;localStorage.removeItem(dismissKey());refreshInstallUi()});
  ...
  function init(){ensureUi();applyTenantManifest();refreshSettingsPreview()}let tries=0;const timer=setInterval(()=>{tries++;ensureUi();if(business()){init();clearInterval(timer)}else if(tries>240)clearInterval(timer)},100);const title=document.querySelector('title');if(title)new MutationObserver(()=>{if(business())applyTenantManifest()}).observe(title,{childList:true});window.SchedulePlusPwa={applyTenantManifest,refreshInstallUi};
})();