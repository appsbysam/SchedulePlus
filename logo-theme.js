(() => {
  const THEME_KEY='schedule_plus_theme_preference';
  const DEFAULT_LOGO='assets/icons/icon.svg';
  const state={target:'dark',image:null,imageUrl:null,drag:false,lastX:0,lastY:0,baseScale:1,scale:1,offsetX:0,offsetY:0,darkFile:null,lightFile:null,darkShape:'square',lightShape:'square',removeLight:false,ready:false};
  const $=id=>document.getElementById(id);
  const getBusiness=()=>window.SchedulePlusBusiness?.business||null;
  ...
  function chooseBusinessLogo(b,theme){
    if(!b)return {url:DEFAULT_LOGO,shape:'square'};
    if(theme==='light')return {url:b.logo_light_url||b.logo_dark_url||b.logo_url||DEFAULT_LOGO,shape:b.logo_light_url?(b.logo_light_shape||'square'):(b.logo_dark_shape||'square')};
