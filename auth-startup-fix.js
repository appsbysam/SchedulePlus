(() => {
  ...
  function maybeShowWhatsNew(){
    ...
    const version=typeof APP_VERSION!=='undefined'?String(APP_VERSION):'';if(!version)return;
    const modal=document.getElementById('updateModal');
    if(version===SILENT_VERSION){
      closeUpdate();
    ...
    const seen=localStorage.getItem(RELEASE_KEY),hasUsed=localStorage.getItem(FIRST_USE_KEY)==='1';
    ...
    const versionLabel=document.getElementById('updateVersion');if(versionLabel)versionLabel.textContent=`Version ${version}`;
    try{modal.showModal()}catch(_){}
  }
  function reconcile(){if(loggedOutScreen())closeHiddenAppDialogs();else maybeShowWhatsNew()}
