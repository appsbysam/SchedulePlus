(()=>{
 ...
 function bind(){const d=modal(),btn=document.getElementById('updateDone');if(!d||!btn||btn.dataset.spModalGuardBound)return;btn.dataset.spModalGuardBound='1';btn.type='button';const close=e=>{e?.preventDefault?.();e?.stopPropagation?.();localStorage.setItem(RELEASE_KEY,version());closeWhatsNew()};btn.onclick=close;d.addEventListener('cancel',close);d.addEventListener('close',()=>{document.documentElement.classList.remove('sp-update-open');document.body.classList.remove('sp-update-open')});
 }
