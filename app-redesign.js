(() => {
  const ICONS={
    ...
    settings:'<svg viewBox="0 0 24 24"><path d="M12 8.2a3.8 3.8 0 1 0 0 7.6 3.8 3.8 0 0 0 0-7.6z"/><path d="m19.4 15 .9 1.7-2.6 2.6-1.7-.9a8 8 0 0 1-2 .8L13.4 21H10.6L10 19.2a8 8 0 0 1-2-.8l-1.7.9-2.6-2.6.9-1.7a8 8 0 0 1-.8-2L2 12.4V9.6L3.8 9a8 8 0 0 1 .8-2l-.9-1.7 2.6-2.6 1.7.9a8 8 0 0 1 2-.8L10.6 1h2.8l.6 1.8a8 8 0 0 1 2 .8l1.7-.9 2.6 2.6-.9 1.7a8 8 0 0 1 .8 2l1.8.6v2.8l-1.8.6a8 8 0 0 1-.8 2z"/></svg>',
  ...
  function ensureViews(){
    ...
    if(dash){dash.classList.add('sp-view');dash.dataset.spView='jobs';}
    if(cal){cal.classList.add('sp-view');cal.dataset.spView='calendar';}
    if(!document.getElementById('spHomeView')){
      ...
      document.getElementById('spViewAllJobs').onclick=()=>showView('jobs');
    }
    if(!document.getElementById('spSettingsView')){
      ...
      s.innerHTML=`<div class="sp-screen-head"><div><h1>Settings</h1><p>All app preferences and configuration</p></div></div><div class="sp-settings-list">
      ${settingCard('business','Business settings','Business details, preferences','spBusinessSettings','sp-ico-business')}
      ${settingCard('config','Configuration','Job types, statuses, priorities','spConfig','sp-ico-config')}
      ${settingCard('users','Users & Staff','Manage team access','spUsers','sp-ico-users')}
      ${settingCard('news',"What's New",'See the latest updates','spWhatsNew','sp-ico-news')}
      ${settingCard('update','Check for Updates',`Current version: ${typeof APP_VERSION!=='undefined'?APP_VERSION:'—'}`,'spUpdates','sp-ico-update')}
      </div>`;
      main.appendChild(s);
      bindSettings();
    }
    if(!document.getElementById('spProfileView')){
      ...
      main.appendChild(p);
      document.getElementById('spMyAccount').onclick=()=>document.getElementById('userProfileMenuBtn')?.click();
      document.getElementById('spSignOut').onclick=()=>document.getElementById('signOutBtn')?.click();
    }
  }
  ...
  function bindSettings(){
    const bind=(id,target)=>document.getElementById(id).onclick=()=>{const el=document.getElementById(target);if(el)el.click();else setTimeout(()=>document.getElementById(target)?.click(),150)};
    bind('spBusinessSettings','businessSettingsBtn');bind('spConfig','businessConfigBtn');bind('spUsers','businessUsersBtn');bind('spWhatsNew','whatsNewBtn');bind('spUpdates','checkUpdatesBtn');
  }
  function showView(name){
    document.querySelectorAll('.sp-view').forEach(v=>v.classList.toggle('active',v.dataset.spView===name));
  ...
  function renderHome(){
    if(!document.getElementById('spHomeView'))return;
    const b=window.SchedulePlusBusiness?.business;
    const logo=b?.logo_dark_url||b?.logo_url||'assets/icons/icon.svg';document.getElementById('spHomeLogo').src=logo;
    const now=new Date(),h=now.getHours();document.querySelector('#spHomeView .hello').textContent=`Good ${h<12?'morning':h<18?'afternoon':'evening'},`;
    document.getElementById('spGreetingDate').textContent=new Intl.DateTimeFormat('en-AU',{weekday:'short',day:'2-digit',month:'short',year:'numeric'}).format(now).replace(',','');
    ...
    const all=Array.isArray(window.jobs)?window.jobs:(typeof jobs!=='undefined'?jobs:[]);const today=(typeof localDate==='function'?localDate():new Date().toISOString().slice(0,10));
    const counts={total:all.length,scheduled:all.filter(j=>j.scheduled_date&&j.status!=='completed').length,progress:all.filter(j=>j.status==='in_progress').length,followup:all.filter(j=>j.status==='waiting').length,completed:all.filter(j=>j.status==='completed').length,overdue:all.filter(j=>j.scheduled_date&&j.scheduled_date<today&&j.status!=='completed').length};
    ...
    const upcoming=all.filter(j=>j.status!=='completed').sort((a,b)=>(a.scheduled_date||'9999').localeCompare(b.scheduled_date||'9999')||(a.scheduled_start||'99:99').localeCompare(b.scheduled_start||'99:99')).slice(0,6);
    const root=document.getElementById('spUpcoming');root.innerHTML=upcoming.length?upcoming.map(j=>homeCard(j)).join(''):'<div class="empty">No upcoming jobs.</div>';
    root.querySelectorAll('.sp-home-job').forEach(el=>el.onclick=e=>{if(e.target.closest('[data-phone]'))return;const j=all.find(x=>x.id===el.dataset.id);if(j&&typeof openJob==='function')openJob(j)});root.querySelectorAll('[data-phone]').forEach(a=>a.onclick=e=>e.stopPropagation());
  }
  function homeCard(j){const phone=j.customer_phone?`<a data-phone class="sp-phone" href="tel:${esc(String(j.customer_phone).replace(/\s+/g,''))}" aria-label="Call">☎</a>`:'';return `<article class="sp-home-job" data-id="${j.id}"><div class="sp-home-job-top"><div><strong>${esc(j.customer_name||'No customer')}</strong><div class="sp-home-job-suburb">${esc(suburb(j))}</div></div><span class="sp-status" data-tone="${toneFor(j)}">${esc(statusText(j))}</span></div><div class="sp-home-job-bottom"><span>${esc(j.title||'Job')}</span><span class="sp-date">▣ ${esc(fmtDate(j.scheduled_date,j.time_of_day||''))}</span>${phone}</div></article>`}

  function enhanceCards(){
    document.querySelectorAll('.compact-job-card').forEach(card=>{const row=card.closest('.swipe-row'),id=row?.dataset.id;const all=Array.isArray(window.jobs)?window.jobs:(typeof jobs!=='undefined'?jobs:[]);const j=all.find(x=>String(x.id)===String(id));if(!j||card.dataset.spEnhanced)return;card.dataset.spEnhanced='1';const top=card.querySelector('.compact-card-top');if(top){const stat=top.querySelector('.compact-status');if(stat)stat.dataset.tone=toneFor(j);const sub=top.querySelector('.compact-suburb');if(sub)sub.remove();}
      const bottom=card.querySelector('.compact-card-bottom');if(bottom){bottom.innerHTML=`<div class="sp-card-line-two"><span class="suburb">${esc(suburb(j))}</span><span class="compact-date">${esc(fmtDate(j.scheduled_date,j.time_of_day||''))}</span></div><div class="sp-card-line-three"><span class="job-type-chip">${esc(j.title||'Job')}</span>${j.customer_phone?`<button class="compact-phone" type="button">${esc(j.customer_phone)}</button>`:'<span class="compact-phone missing">No phone</span>'}</div>`;}
    });
  }
  ...
  function wrapRenderers(){
    if(typeof render==='function'&&!render.__spRedesign){const old=render;render=function(){old();setTimeout(()=>{enhanceCards();renderHome()},0)};render.__spRedesign=true;}
    if(typeof renderCalendar==='function'&&!renderCalendar.__spRedesign){const oldc=renderCalendar;renderCalendar=function(){oldc();setTimeout(decorateCalendar,0)};renderCalendar.__spRedesign=true;}
  }
