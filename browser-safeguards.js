(() => {
  ...
  function ensureUi(){
    if(!document.getElementById('browserSupportWarning')){
      const warning=document.createElement('div');warning.id='browserSupportWarning';warning.className='browser-support-warning hidden';warning.innerHTML='<div><strong>Unsupported browser</strong><span>Schedule+ is designed for Chrome and Safari. Some functions, including app installation, may not work correctly in this browser.</span></div><button type="button" aria-label="Dismiss browser warning">×</button>';document.body.appendChild(warning);warning.querySelector('button').addEventListener('click',()=>{localStorage.setItem(WARNING_KEY,String(Date.now()));warning.classList.add('hidden')});
    }
  ...
  function neutraliseUnsupportedModals(){
    if(supported)return;
    ...
    const help=document.getElementById('pwaInstallHelp');if(help){try{if(help.open)help.close()}catch(_){}help.removeAttribute('open')}
    const crop=document.getElementById('pwaCropDialog');if(crop){try{if(crop.open)crop.close()}catch(_){}crop.removeAttribute('open')}
  ...
  function init(){ensureUi();neutraliseUnsupportedModals();showWarning();if(!supported){setTimeout(neutraliseUnsupportedModals,250);setTimeout(neutraliseUnsupportedModals,1000)}}
